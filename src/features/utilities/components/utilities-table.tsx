...
import { useUtilitiesColumns } from './utilities-columns'

type UtilityWithChildren = Utility & { subRows?: UtilityWithChildren[] }

/** Flatten nested API response (which may already have children[]) into a flat list */
function flattenTree(items: any[]): Utility[] {
  const result: Utility[] = []
  for (const item of items) {
    const { children, ...rest } = item
    result.push(rest as Utility)
    if (children?.length) {
...
function buildTree(flatData: Utility[]): UtilityWithChildren[] {
  const map = new Map<number, UtilityWithChildren>()
  const roots: UtilityWithChildren[] = []
...
export function UtilitiesTable({
  ...
  const columns = useMemo(() => useUtilitiesColumns(t), [t])
  ...
  const table = useReactTable({
    ...
    getSubRows: (row) => (row as UtilityWithChildren).subRows,
