...
export function UtilitiesActionDialog() {
  ...
  useEffect(() => {
    if (isEditMode && currentRow) {
      ...
      form.reset({ name: '', parent_id: null, media_items: [] })
      setUploadedMedia([])
      setPendingFiles([])
    }
  }, [isEditMode, currentRow, open, form])
  ...
  const onSubmit = async (data: UtilityFormValues) => {
    const media_items = uploadedMedia.map((m) => ({
      media_id: Number(m.media_id),
      position: Math.floor(m.position),
    }))
    try {
      if (isEditMode && currentRow) {
        await updateUtility.mutateAsync({
          id: currentRow.id,
          data: {
            name: data.name ?? null,
  ...
  const isLoading = createUtility.isPending || updateUtility.isPending
  const utilities = Array.isArray(utilitiesData)
    ? utilitiesData
    : (utilitiesData?.data ?? [])

  // Filter out current utility from parent options
  const availableParents = currentRow
    ? utilities.filter((u) => u.id !== currentRow.id)
    : utilities
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className='space-y-6'>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>{t('parent') || 'Parent Utility'}</FormLabel>
                  <Select
                  ...
                  >
                    <FormControl>
                      <SelectTrigger>
                        <SelectValue
                          placeholder={
                            t('select_parent') || 'Select parent utility'
                          }
                        />
                      </SelectTrigger>
                    </FormControl>
                    <SelectContent>
                      ...
                      {availableParents.map((utility) => (
                        <SelectItem key={utility.id} value={String(utility.id)}>
                          {utility.name || `Utility ${utility.id}`}
                        </SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
